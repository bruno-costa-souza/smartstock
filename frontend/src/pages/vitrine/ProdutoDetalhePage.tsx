...
export function ProdutoDetalhePage() {
  ...
  if (isError || !produto) {
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="text-center glass-card p-8 animate-fade-up">
          ...
          <button
            onClick={() => navigate('/')}
            className="mt-4 text-brand-300 text-sm font-medium hover:text-brand-200 transition"
          >
            Voltar para a loja
          </button>
        </div>
  ...
  const baixoEstoque = ESTOQUE_ATIVO && produto.estoque > 0 && produto.estoque <= produto.estoqueMin;
    ...
    <div className="min-h-screen">
      ...
      <header className="glass-strong border-x-0 border-t-0 sticky top-0 z-30 shadow-lg shadow-black/30">
        <div className="flex items-center gap-3 px-4 h-14 max-w-4xl mx-auto">
          <button
            onClick={() => navigate(-1)}
            className="w-9 h-9 flex items-center justify-center rounded-xl btn-ghost flex-shrink-0"
          >
            <ArrowLeft size={18} />
          </button>
      ...
      <div className="max-w-4xl mx-auto md:px-4 md:py-6">
        <div className="md:flex md:gap-6 md:items-start">
          ...
          {/* Imagem */}
          <div className="md:w-5/12 md:flex-shrink-0 animate-fade-up">
          ...
          {/* Info + CTA */}
          <div className="md:flex-1 space-y-3 md:space-y-4 animate-fade-up">

            {/* Info panel */}
            <div className="glass border-x-0 md:border-x md:rounded-2xl px-5 py-6 space-y-4">

              {/* Categoria */}
              <span className="inline-block text-xs font-bold text-brand-300 bg-brand-500/20 border border-brand-500/30 px-3 py-1 rounded-full uppercase tracking-wider">
                {produto.categoria}
              </span>
            ...
            {/* CTA */}
            <div className="glass border-x-0 md:border-x md:rounded-2xl px-4 py-4 md:p-5 space-y-3">
              {semEstoque ? (
                ...
                <button
                  onClick={() => navigate(-1)}
                  className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl btn-whatsapp text-base"
                >
                  <Check size={20} />
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl btn-primary text-base"
                >
              ...
              <button
                onClick={() => navigate('/')}
                className="w-full py-3.5 rounded-2xl btn-ghost text-sm font-semibold"
              >
                Continuar comprando
              </button>
            </div>
...
function DetalheSkeleton() {
  return (
    <div className="min-h-screen animate-pulse">
      <div className="glass border-x-0 border-t-0 h-14" />
      <div className="max-w-4xl mx-auto md:px-4 md:py-6">
        <div className="md:flex md:gap-6">
          ...
          <div className="md:flex-1 space-y-3 mt-3 md:mt-0">
            ...
            <div className="glass border-x-0 md:border-x md:rounded-2xl px-4 py-4 space-y-3">
