    ...
    <div className="p-4 md:p-6">
      ...
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Gerencie o catálogo da papelaria</p>
        </div>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 md:px-4 rounded-lg text-sm font-medium transition-colors flex-shrink-0"
        >
          <Plus size={16} />
      ...
      <input
        ...
        className="w-full max-w-sm mb-4 px-3 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:border-blue-500"
      />

      {/* ── Mobile: lista de cards ─────────────────────────── */}
      <div className="md:hidden space-y-2">
        {produtos.length === 0 ? (
          <p className="text-center text-gray-500 text-sm py-10">Nenhum produto cadastrado</p>
        ) : (
          produtos.map((p) => {
            const imgUrl = getImageUrl(p.imagemUrl);
            return (
              <div key={p.id} className="bg-gray-900 rounded-xl border border-gray-800 flex items-center gap-3 p-3">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button
                    onClick={() => openEdit(p)}
                    className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-gray-700 text-gray-400 hover:text-white transition"
                  >
                    <Pencil size={15} />
                  </button>
                  <button
                    onClick={() => confirmDelete(p)}
                    className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-red-900/30 text-gray-400 hover:text-red-400 transition"
                  >
                    <Trash2 size={15} />
                  </button>
                </div>
      ...
      <div className="hidden md:block bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
              ...
              return (
                <tr key={p.id} className="border-b border-gray-800 hover:bg-gray-800/40 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {p.descricao && <p className="text-gray-500 text-xs truncate max-w-[200px]">{p.descricao}</p>}
                  </td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 bg-blue-900/40 text-blue-400 rounded-full text-xs">{p.categoria}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    <span className="text-gray-600 text-xs"> / min {p.estoqueMin}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openEdit(p)} className="p-1.5 hover:bg-gray-700 rounded-lg text-gray-400 hover:text-white transition-colors">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => confirmDelete(p)} className="p-1.5 hover:bg-red-900/30 rounded-lg text-gray-400 hover:text-red-400 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
            ...
            {produtos.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-10 text-center text-gray-500">Nenhum produto cadastrado</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-4">
          <button disabled={page === 1} onClick={() => setPage((p) => p - 1)} className="px-3 py-1.5 rounded-lg border border-gray-700 text-sm text-gray-400 disabled:opacity-40 hover:border-gray-500">Anterior</button>
          <span className="px-3 py-1.5 text-sm text-gray-500">{page} / {totalPages}</span>
          <button disabled={page === totalPages} onClick={() => setPage((p) => p + 1)} className="px-3 py-1.5 rounded-lg border border-gray-700 text-sm text-gray-400 disabled:opacity-40 hover:border-gray-500">Próxima</button>
        </div>
      )}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div role="dialog" aria-modal="true" aria-labelledby="produto-modal-title" className="bg-gray-900 rounded-t-2xl sm:rounded-2xl border border-gray-800 w-full sm:max-w-lg max-h-[95vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-800 sticky top-0 bg-gray-900 z-10">
              ...
              <button onClick={closeModal} className="p-1.5 hover:bg-gray-800 rounded-lg text-gray-400"><X size={18} /></button>
            </div>

            <div className="p-5 space-y-4">
              {/* Upload de imagem */}
              <div>
                <label className="block text-xs text-gray-400 mb-2">Foto do produto</label>
                <div
                  className="aspect-video rounded-xl border-2 border-dashed border-gray-700 flex items-center justify-center cursor-pointer hover:border-blue-500 transition-colors overflow-hidden bg-gray-800"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Estoque inicial</label>
            ...
            <div className="flex gap-3 p-5 border-t border-gray-800 sticky bottom-0 bg-gray-900">
              <button onClick={closeModal} className="flex-1 py-3 rounded-xl border border-gray-700 text-sm text-gray-400 hover:bg-gray-800 transition-colors">Cancelar</button>
              <button
                onClick={() => saveMutation.mutate(form)}
                disabled={saveMutation.isPending || !form.nome || !form.preco || !form.categoria}
                className="flex-1 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-50 transition-colors"
              >
